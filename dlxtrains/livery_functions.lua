	...
	if item_name == "dlxtrains:wagon_updater" or item_name == "dlxtrains:age_selector" or item_name == "dlxtrains:livery_selector" then
		...
		if livery_count > 0 then
			...
			if item_name == "dlxtrains:wagon_updater" then
				minetest.sound_play("dlxtrains_wagon_updater_tool", {pos = pos, gain = 0.6, max_hear_distance = 11})
			else
				...
				if item_name == "dlxtrains:age_selector" then
					...
					minetest.sound_play("dlxtrains_age_selection_tool", {pos = pos, gain = 0.8, max_hear_distance = 11})
				elseif livery_count > 1 then
					...
					minetest.sound_play("dlxtrains_livery_selection_tool", {pos = pos, gain = 0.6, max_hear_distance = 11})
				end
			end
			wagon:set_textures(data)
